...
SEED_DIR="${ROOT_DIR}/desktop/seed-images"
...
# Load IMAGE_* and GHCR_OWNER from .env, same as the compose/publish flow
...
# By default the source comes from IMAGE_* (or :local), but the seed always writes :local tags
if [ "$#" -eq 0 ]; then
  SOURCE_IMAGES=(
...
# Cleanup old archives before a new export
...
for idx in "${!SOURCE_IMAGES[@]}"; do
  ...
  # If the source is not :local, add a :local alias so the launcher finds local_tag after docker load
