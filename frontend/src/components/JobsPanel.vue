      ...
      >
        <div class="flex flex-wrap items-center justify-between gap-2">
          ...
          <div class="inline-flex items-center gap-2">
            <span class="inline-flex items-center gap-2 text-xs uppercase text-slate-300">
              <span class="h-2 w-2 rounded-full" :class="job.status === 'succeeded' ? 'bg-emerald-400' : (job.status === 'failed' || job.status === 'canceled') ? 'bg-red-400' : 'animate-pulse bg-amber-400'" />
              {{ job.status }}
            </span>
